...
import net.sorenon.mixin.MultiPlayerScreenMixin;
...
public class XrEditBoxKeyboard extends XrAbstract2DKeyboard {
    ...
    public void returnButton(Button instance) {
        ...
        if (_editScreen.getParentScreen().getClass() == DirectJoinServerScreen.class) {
            ServerData server = new ServerData(_placeholderField.getValue(), "Server", ServerData.Type.OTHER);
            ConnectScreen.startConnecting(_editScreen.getParentScreen(), Minecraft.getInstance(), ServerAddress.parseString(_placeholderField.getValue()), server, false, null);
        } else if (_editScreen.getParentScreen().getClass() == JoinMultiplayerScreen.class) {
            ServerData server = new ServerData(_placeholderField.getValue(), _placeholderField.getValue(), ServerData.Type.OTHER);

...
import net.sorenon.gui.XrChatScreen;
...
public class XrChatKeyboard extends XrAbstract2DKeyboard {
    ...
    public void returnButton(Button instance) {
        if (_chatBox.getValue().isEmpty()) {
            _chatScreen.onClose();
        } else {
            if(_chatBox.getValue().startsWith("/")) {
                Minecraft.getInstance().player.connection.sendCommand(_chatBox.getValue().substring(1));
                _chatScreen.onClose();
            } else {
                Minecraft.getInstance().player.connection.sendChat(_chatBox.getValue());
                _chatScreen.onClose();
            }
        }
    }

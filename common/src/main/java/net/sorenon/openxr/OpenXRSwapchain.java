...
public class OpenXRSwapchain implements AutoCloseable {
    ...
    public OpenXRSwapchain(XrSwapchain handle, OpenXRSession session, int format, int width, int height) {
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            StructBuffer swapchainImageBuffer;
            if (MCOpenXRClient.android) {
                swapchainImageBuffer = XrSwapchainImageOpenGLESKHR.calloc(imageCount, stack);
                for (Object image : swapchainImageBuffer) {
                    ((XrSwapchainImageOpenGLESKHR) image).type(KHROpenGLESEnable.XR_TYPE_SWAPCHAIN_IMAGE_OPENGL_ES_KHR);
                }
            } else {
                swapchainImageBuffer = XrSwapchainImageOpenGLKHR.calloc(imageCount, stack);
                for (Object image : swapchainImageBuffer) {
                    ((XrSwapchainImageOpenGLKHR) image).type(KHROpenGLEnable.XR_TYPE_SWAPCHAIN_IMAGE_OPENGL_KHR);
                }
            }
            ...
            for (int i = 0; i < imageCount; i++) {
                ...
                if (MCOpenXRClient.android) {
                    arrayImages[i] = ((XrSwapchainImageOpenGLESKHR) openxrImage).image();
                } else {
                    arrayImages[i] = ((XrSwapchainImageOpenGLKHR) openxrImage).image();
                }

...
public class OpenXRInstance implements AutoCloseable {
        ...
        while (event != null) {
            switch (event.type()) {
                ...
                case XR10.XR_TYPE_EVENT_DATA_SESSION_STATE_CHANGED: {
                    return MCOpenXRClient.OPEN_XR_STATE.session.stateChanged(XrEventDataSessionStateChanged.create(event.address()));
                }
    ...
    @Override
    public void close() {
        var session = MCOpenXRClient.OPEN_XR_STATE.session;
        if (session != null) {
            session.close();
        }

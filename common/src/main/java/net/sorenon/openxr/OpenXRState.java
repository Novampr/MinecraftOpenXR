...
public class OpenXRState {
    ...
    public void tryInitialize() {
        ...
        try {
            ...
            MCOpenXRClient.MCXR_GAME_RENDERER.setSession(session);
        } catch (Exception e) {
            ...
            if (instance != null) instance.close();
            instance = null;
            MCOpenXRClient.OPEN_XR_STATE.session = null;
        }
    }
    ...
    public OpenXRInstance createOpenXRInstance() throws XrException {
        try (MemoryStack stack = stackPush()) {
            ...
            if (missingOpenGL) {
                throw new XrException(0, "OpenXR runtime does not support OpenGLES, no alternatives found!");
            }

            XrApplicationInfo applicationInfo = XrApplicationInfo.calloc(stack);
            applicationInfo.apiVersion(XR10.XR_CURRENT_API_VERSION);
            applicationInfo.applicationName(stack.UTF8("MinecraftOpenXR : Minecraft VR"));
    ...
    public boolean loop() {
        ...
        if (session.running) {
            ...
            MCOpenXRClient.MCXR_GAME_RENDERER.renderFrame(disabled);
            return !MCOpenXRClient.MCXR_GAME_RENDERER.isXrMode();
        }
        return true;
    }

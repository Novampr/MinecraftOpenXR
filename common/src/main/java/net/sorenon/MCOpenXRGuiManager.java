...
import net.sorenon.rendering.UnownedTexture;
import net.sorenon.rendering.MCXRCamera;
import org.jetbrains.annotations.Nullable;
import org.joml.*;

public class MCOpenXRGuiManager {

    public final int guiFramebufferWidth = 1980;
    public final int guiFramebufferHeight = 1080;

    public final ResourceLocation guiRenderTexture = ResourceLocation.fromNamespaceAndPath("mcxr", "net/sorenon/mcxr/play/gui");
    ...
    public double calcGuiScale() {
        ...
        int scale;
        scale = 1;
        while (scale != guiScale && scale < guiFramebufferHeight && guiFramebufferWidth / (scale + 1) >= 320 && guiFramebufferHeight / (scale + 1) >= 240) {
            ++scale;
        }
    ...
    public void resetTransform() {
        ...
        if (camera.isInitialized()) {
            orientation = camera.rotation().get(new Quaterniond());
            position = JOMLUtil.convert(MCOpenXRClient.viewSpacePoses.getUnscaledPhysicalPose().getPos().add(orientation.transform(new Vector3f(0, -0.5f, 1))));
            needsReset = false;
        } else {
            needsReset = true;
        }

...
import net.sorenon.MCOpenXRNativeLoader;
import org.lwjgl.*;
...
public final class XR {
        ...
        GlobalCommands(FunctionProvider library) {
            ...
            if (xrInitializeLoaderKHR != NULL) {
                try (MemoryStack stack = stackPush()) {
                    long context = memGetAddress(MCOpenXRNativeLoader.getApplicationActivityPtr());
                    long jvm = memGetAddress(MCOpenXRNativeLoader.getJVMPtr());

                    var createInfo = XrInstanceCreateInfoAndroidKHR
                            .calloc(stack)
                            .type$Default()
                            .next(NULL)
                            .applicationVM(jvm)
    ...
    static Set<String> getEnabledExtensionSet(long apiVersion, PointerBuffer extensionNames) {
